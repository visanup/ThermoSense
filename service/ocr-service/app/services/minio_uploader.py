...
class MinioUploader:
    ...
    @retry(total_tries=3, initial_delay=0.5, backoff=2.0)
    def download_processed(self, object_name: str) -> bytes:
        logger.info("Downloading processed object %s from bucket %s", object_name, Config.MINIO_PROCESSED_BUCKET)
        try:
            obj = self.client.get_object(Config.MINIO_PROCESSED_BUCKET, object_name)
            data = obj.read()
            obj.close()
            obj.release_conn()
            logger.info("Downloaded processed object %s (%d bytes)", object_name, len(data))
            return data
        except Exception as e:
            logger.exception("Failed to download processed object %s", object_name)
            raise
